...
{// pgm est true si l'image est pgm
    ...
    if (MCU->blocs != NULL){
        for (uint8_t i = 0; i<MCU->bloc_col*MCU->bloc_line; i++){
            MCU_YCbCr->blocs[i] = *conversion(MCU->blocs[i]);
        }    
    }
    else{
        MCU_YCbCr->blocs[0] = *conversion(MCU); // j'ai pas compris ce else
    }
    //free_MCU(*MCU);
    return MCU_YCbCr;
    ...
    if (MCU->blocs != NULL){
        for (uint8_t i = 0; i<MCU->bloc_col*MCU->bloc_line; i++){
            MCU_YCbCr->blocs[i] = *conversion_pgm(MCU->blocs[i]->matrix);
        }    
    }else{
        MCU_YCbCr->blocs[0] = *conversion_pgm(MCU->matrix); // ce else aussi je l'ai pas compris
        }
    //free_MCU(*MCU);
    return MCU_YCbCr;
    }
...
struct matrix_MCU_YCbCr *matrix_MCU_to_YCbCr(struct matrix_MCU matrix_MCU)
{
    ...
    for (uint32_t i = 0; i<matrix_MCU_YCbCr->bloc_line; i++){
        ...
        for (uint32_t j = 0; j<matrix_MCU_YCbCr->bloc_col; j++){
            matrix_MCU_YCbCr->matrice[i][j] = *MCU_to_YCbCr(&(matrix_MCU.matrice[i][j]));
        }
    }
    //free_img_MCU(matrix_MCU);
