...
void *code_AC_RLE(int16_t *vector, struct bitstream *bitstream, struct huff_table *hf)
{
    ...
    for (uint32_t i= 0; i< non_zero_elm; i++){
        ...
        //printf("element courant est %04hx et marqueur est %i\n", vector[marqueur_vector], marqueur_vector);
    ...
    bitstream_write_bits(bitstream, eob, *path_long_marker, false); // on termine le bloc AC et donc on écrite le marqueur
    free(path_long);
...
*/
struct MCU_vector *MCU_to_vector(struct MCU_YCbCr *MCU, struct fact_echant *fact1, struct fact_echant *fact2)
{
    ...
    for (uint32_t i = 0; i<(fact2->h*fact2->v) ; i++){
...
struct DC_MCU *code_DC(struct MCU_vector *MCU_vect)
{
    ...
    for (uint32_t i = 0; i<len_Y; i++){
        if (flag_Y == 1){
            ...
            DC_vect->Y[i] = MCU_vect->vector_Y.vector[i][0] - MCU_vect->vector_Y.vector[i-1][0];
        }      
    }
    for (uint32_t i = 0; i<len_Cb; i++){
        if (flag_Cb == 1){
            ...
            DC_vect->Cb[i] = MCU_vect->vector_Cb.vector[i][0] - MCU_vect->vector_Cb.vector[i-1][0];
        }      
    }
    for (uint32_t i = 0; i<len_Cr; i++){
        if (flag_Cr == 1){
            ...
            DC_vect->Cr[i] = MCU_vect->vector_Cr.vector[i][0] - MCU_vect->vector_Cr.vector[i-1][0];
        }      
    }
    return DC_vect;
}
