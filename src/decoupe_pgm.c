...
{   
    if (MCU.blocs == NULL){
        ...
        for (uint8_t v = 0; v<(MCU.bloc_col*MCU.bloc_line); v++){
            free_MCU(*(MCU.blocs[v]));
            free(MCU.blocs[v]);
        }
    }
    free(MCU.blocs);
...
{   
    for (uint8_t l = 0; l<(MCU.col*MCU.lines); l++){
        if (MCU.blocs[l].Y != NULL) free_matrix(MCU.blocs[l].Y, 8);
        if (MCU.blocs[l].Cb != NULL) free_matrix(MCU.blocs[l].Cb, 8);
        if (MCU.blocs[l].Cr != NULL) free_matrix(MCU.blocs[l].Cr, 8);
    }
    free(MCU.blocs);
    //free(&MCU);
}
...
{
    for (uint8_t x = 0; x<img.MCU_lines; x++){
        for (uint8_t y = 0; y<img.MCU_col; y++){
            free_MCU(img.matrice[x][y]);
            //free(&(img.matrice[i][j]));
        }
    }
    for (uint8_t z = 0; z<img.MCU_lines; z++) free(img.matrice[z]);
    free(img.matrice);
}
...
struct matrix_MCU image_to_MCU(struct matrix matrix_import, uint32_t bloc_lines, uint32_t bloc_col)
{
    ...
    //free_matrix(matrix_import.matrice, matrix_import.lines);
    free(sub_matrix);
...
/*int main(void)
{
    int16_t **tab = calloc(16, sizeof(int16_t *));
    for (uint32_t i = 0; i<16; i++){
        tab[i] = calloc(16, sizeof(int16_t));
        for (uint32_t j = 0; j<16; j++){
            if (j == 15) tab[i][j] = 1;
            else if (i == 16) tab[i][j] = 0;
        }
    }
